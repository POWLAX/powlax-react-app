-- POWLAX User Profile Population with Proper Name Handling (FIXED)
-- Created: 2025-01-16
...
SET 
  ...
  first_name = CASE
    WHEN email = 'player1@example.com' THEN 'Johnny'  -- Coach's son
    WHEN email = 'powlax.patrick@gmail.com' THEN 'Patrick'
    WHEN email = 'powlax.coach@gmail.com' THEN 'Mike'  -- Coach's first name
  ...
  END,
  last_name = CASE
    WHEN email = 'player1@example.com' THEN 'Johnson'  -- Same last name as coach
    WHEN email = 'powlax.patrick@gmail.com' THEN 'Chapla'
    WHEN email = 'powlax.coach@gmail.com' THEN 'Johnson'  -- Coach's last name
  ...
  account_type = CASE
    ...
    WHEN email = 'powlax.coach@gmail.com' THEN 'parent'  -- Parent account
    WHEN email = 'player1@example.com' THEN 'child'  -- Child account
    ELSE 'individual'
  END,
...
-- Update role to remove admin access (roles is text[] not jsonb)
UPDATE users
SET 
  role = 'disabled',
  roles = ARRAY['disabled']::text[],  -- Fixed: Use text array instead of jsonb
  metadata = jsonb_set(
...
-- ==========================================
-- STEP 4: CREATE FAMILY RELATIONSHIP - COACH AND SON
-- ==========================================

-- Create a family account for Coach Johnson and his son Player 1
DO $$
DECLARE
  v_coach_id UUID;
  v_player1_id UUID;
...
BEGIN
  -- Get user IDs
  SELECT id INTO v_coach_id FROM users WHERE email = 'powlax.coach@gmail.com';
  ...
  IF v_coach_id IS NOT NULL AND v_player1_id IS NOT NULL THEN
    -- Create family account for the Johnson family
    ...
    VALUES (
      ...
      'Coach''s son - plays on the team'
    );
    
    RAISE NOTICE 'Created family account for Coach Johnson and Player 1 (son)';
  END IF;
END $$;
...
SELECT 
  ...
  u_child.display_name as "Child Display Name",
  u_child.first_name || ' ' || u_child.last_name as "Child Real Name",
  pcr.notes as "Relationship Notes"
...
-----------------
Johnson Family:
- Parent/Coach: powlax.coach@gmail.com (Mike Johnson) - Team Coach
- Child: player1@example.com (Johnny Johnson) - Coach's son, plays on team
  * Display Name: "Player 1" (for gaming/leaderboards)
  * Real Name: "Johnny Johnson" (what coach sees on roster)

Patrick (Admin):
- powlax.patrick@gmail.com - Platform admin, not in a family

DISPLAY NAME VS REAL NAME USAGE:
---------------------------------
Frontend Implementation:
// For public leaderboards and gamification:
const displayName = user.display_name;  // "Player 1"

// For coach viewing team roster:
const playerName = `${user.first_name} ${user.last_name}`;  // "Johnny Johnson"

// Special case - coach viewing own child:
// Coach sees "Johnny Johnson" on roster, but "Player 1" on public leaderboard

REAL vs TEST Accounts:
----------------------
Real Accounts (3):
- powlax.patrick@gmail.com (Patrick Chapla - Platform Admin)
- powlax.coach@gmail.com (Mike Johnson - Team Coach & Parent)
- player1@example.com (Johnny Johnson - Player & Coach's Son)
...
Disabled Account (1):
- wpps-support@example.com (WordPress support - disabled)
*/