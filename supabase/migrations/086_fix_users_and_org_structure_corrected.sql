-- POWLAX Fix User Accounts and Set Up Organization Structure (CORRECTED)
-- Created: 2025-01-16
-- Purpose: Fix user classifications, set up family relationships, and map BuddyBoss groups to club/team structure
-- Uses existing team_teams table and clubs table
...
VALUES (
  'a22ad3ca-9f1c-4c4f-9163-021c6af927ac',  -- Use the existing club_id from team_teams
  'Your Club OS',
  NOW(),
  NOW()
)
...
BEGIN
  ...
  SELECT id INTO v_varsity_team_id FROM team_teams WHERE name = 'Your Varsity Team HQ';
  SELECT id INTO v_jv_team_id FROM team_teams WHERE name = 'Your JV Team HQ';
  SELECT id INTO v_eighth_team_id FROM team_teams WHERE name = 'Your 8th Grade Team HQ';
...
LEFT JOIN team_teams t ON t.club_id = c.id
...
JOIN team_teams t ON tm.team_id = t.id
...
Teams (team_teams table):
-------------------------
Your Varsity Team HQ → ID: d6b72e87-8fab-4f4c-9921-260501605ee2
- Players: asia-mills, cali-runolfsdottir (Class of 2025)
- Head Coach: powlax_coach (Mike Johnson)
- Admin: powlax_patrick

Your JV Team HQ → ID: 43642a09-17b6-4813-b9ea-d69a2cd7ad6a
- Players: demario-kertzmann, jaida-grimes (Class of 2026)
- Head Coach: powlax_coach (Mike Johnson)
- Admin: powlax_patrick

Your 8th Grade Team HQ → ID: 044c362a-1501-4e38-aaff-d2ce83381a85
