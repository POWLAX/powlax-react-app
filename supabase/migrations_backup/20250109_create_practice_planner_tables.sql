-- Migration: Create Practice Planner Phase 2 Tables
-- Date: 2025-01-09
...
CREATE INDEX idx_powlax_images_user_drill ON powlax_images(user_drill_id);
CREATE INDEX idx_powlax_images_strategy ON powlax_images(strategy_id);
CREATE INDEX idx_powlax_images_uploaded_by ON powlax_images(uploaded_by);
...
CREATE TABLE IF NOT EXISTS practice_plans (
  ...
  selected_strategies UUID[] DEFAULT ARRAY[]::UUID[],
...
CREATE INDEX idx_practice_plans_date ON practice_plans(practice_date);
CREATE INDEX idx_practice_plans_template ON practice_plans(template) WHERE template = true;
CREATE INDEX idx_practice_plans_parent ON practice_plans(parent_id);
...
CREATE TABLE IF NOT EXISTS practice_plan_drills (
  ...
  drill_id UUID REFERENCES powlax_drills(id) ON DELETE CASCADE, -- Can reference powlax_drills
  user_drill_id INTEGER REFERENCES user_drills(id) ON DELETE CASCADE, -- Can reference user_drills
...
CREATE INDEX idx_practice_plan_drills_drill ON practice_plan_drills(drill_id);
CREATE INDEX idx_practice_plan_drills_user_drill ON practice_plan_drills(user_drill_id);
CREATE INDEX idx_practice_plan_drills_order ON practice_plan_drills(practice_plan_id, order_index);
...
-- Create indexes for performance
CREATE INDEX idx_practice_templates_age_group ON practice_templates(age_group);
CREATE INDEX idx_practice_templates_category ON practice_templates(category);
CREATE INDEX idx_practice_templates_public ON practice_templates(is_public) WHERE is_public = true;
CREATE INDEX idx_practice_templates_official ON practice_templates(is_official) WHERE is_official = true;
CREATE INDEX idx_practice_templates_org ON practice_templates(organization_id);
CREATE INDEX idx_practice_templates_created_by ON practice_templates(created_by);
