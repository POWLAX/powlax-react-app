#!/usr/bin/env npx tsx
/**
 * PATRICK'S DASHBOARD DATA SEEDER
 * Sets up Patrick's admin account with children and gamification data
 * This will make the dashboard show real, connected data
 */
...
async function seedPatrickDashboardData() {
  console.log('🚀 Starting Patrick\'s Dashboard Data Seeding...\n')

  try {
    // 1. First, update Patrick's account to have family admin type for full features
    ...
    if (updateError) {
      ...
      console.log('✅ Updated Patrick\'s account to family admin')
    }

    // 2. Create child accounts for Patrick
    ...
    // 5. Set up gamification currencies
    ...
    // 6. Create point wallets for each child
    console.log('\n💳 Creating point wallets for children...')
    for (const [key, childId] of Object.entries(childIds)) {
      // Create wallets for each currency
      for (const currency of currencies) {
        const walletId = randomUUID()
        const balance = currency.id === 'points' 
          ? Math.floor(Math.random() * 5000) + 1000  // Random points between 1000-6000
          : currency.id === 'xp'
          ? Math.floor(Math.random() * 10000) + 2000 // Random XP between 2000-12000
          : Math.floor(Math.random() * 100) + 10      // Random coins between 10-110
        ...
        if (error && !error.message.includes('duplicate')) {
          ...
          console.log(`✅ Created ${currency.name} wallet for child with balance: ${balance}`)
        }
      }
    }

    // 7. Create some point transactions for history
    ...
    // 8. Create badges and assign some to children
    console.log('\n🏅 Creating badges and achievements...')
    const badgeTypes = [
      { name: 'First Workout', description: 'Complete your first workout', icon: '🎯' },
      { name: 'Week Warrior', description: 'Complete 7 workouts in a week', icon: '💪' },
      { name: 'Drill Master', description: 'Complete 50 drills', icon: '⭐' },
      { name: 'Consistency King', description: '30-day streak', icon: '👑' },
      { name: 'Speed Demon', description: 'Complete a workout in under 10 minutes', icon: '⚡' },
      { name: 'Wall Ball Expert', description: 'Master all wall ball drills', icon: '🏐' }
    ]
    ...
    // 11. Set up player ranks for children
    console.log('\n🎖️ Setting up player ranks...')
    const ranks = ['Rookie', 'Starter', 'Varsity', 'All-Star', 'Elite']
    
    for (const [index, childId] of Object.values(childIds).entries()) {
      const rankIndex = Math.min(index, ranks.length - 1)
      const { error } = await supabase
        .from('user_rank_progress_powlax')
        .upsert({
          id: randomUUID(),
          user_id: childId,
          current_rank: ranks[rankIndex],
          rank_points: (rankIndex + 1) * 1000,
          next_rank: ranks[Math.min(rankIndex + 1, ranks.length - 1)],
          points_to_next: 1000,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString()
        }, { onConflict: 'user_id' })
      ...
      if (error && !error.message.includes('duplicate')) {
        ...
        console.log(`✅ Set rank for child: ${ranks[rankIndex]}`)
      }
    }

    console.log('\n✨ Dashboard data seeding complete!')
    console.log('\n📊 Summary:')
    console.log('  - Patrick\'s admin account updated to family admin')
    console.log('  - 3 child accounts created and linked')
    console.log('  - Family account established')
    console.log('  - Gamification system fully populated:')
    ...
    console.log('    • Player ranks established')
    ...
    console.log('2. The dashboard will now show real data instead of mock data')
    console.log('3. You can view as parent to see children\'s gamification stats')
    console.log('4. All gamification data is now properly connected and will display')
    
    console.log('\n👨‍👧‍👦 Your Children:')
    console.log('  - Alex Chapla (14-17 years, Attack)')
