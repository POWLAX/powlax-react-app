-- POWLAX Complete Tables Setup (Fixed)
...
CREATE INDEX IF NOT EXISTS idx_points_balance_user_powlax ON user_points_balance_powlax(user_id);
CREATE INDEX IF NOT EXISTS idx_points_transactions_user_powlax ON points_transactions_powlax(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_badge_progress_user_powlax ON user_badge_progress_powlax(user_id);
CREATE INDEX IF NOT EXISTS idx_rank_progress_user_powlax ON user_rank_progress_powlax(user_id);

-- ============================================
-- SECTION 3: ROW LEVEL SECURITY (Fixed)
-- ============================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'drills_powlax' AND policyname = 'Allow anonymous read') THEN
    CREATE POLICY "Allow anonymous read" ON drills_powlax FOR SELECT TO anon USING (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'wall_ball_powlax' AND policyname = 'Allow anonymous read') THEN
    CREATE POLICY "Allow anonymous read" ON wall_ball_powlax FOR SELECT TO anon USING (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'lessons_powlax' AND policyname = 'Allow anonymous read') THEN
    CREATE POLICY "Allow anonymous read" ON lessons_powlax FOR SELECT TO anon USING (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'drill_strategy_map_powlax' AND policyname = 'Allow anonymous read') THEN
    CREATE POLICY "Allow anonymous read" ON drill_strategy_map_powlax FOR SELECT TO anon USING (true);
  END IF;
  
  -- Authenticated user policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'drills_powlax' AND policyname = 'Allow authenticated full access') THEN
    CREATE POLICY "Allow authenticated full access" ON drills_powlax FOR ALL TO authenticated USING (true) WITH CHECK (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'wall_ball_powlax' AND policyname = 'Allow authenticated full access') THEN
    CREATE POLICY "Allow authenticated full access" ON wall_ball_powlax FOR ALL TO authenticated USING (true) WITH CHECK (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'lessons_powlax' AND policyname = 'Allow authenticated full access') THEN
    CREATE POLICY "Allow authenticated full access" ON lessons_powlax FOR ALL TO authenticated USING (true) WITH CHECK (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'drill_strategy_map_powlax' AND policyname = 'Allow authenticated full access') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'point_types_powlax' AND policyname = 'Anyone can view point types') THEN
    CREATE POLICY "Anyone can view point types" ON point_types_powlax FOR SELECT USING (true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'badges_powlax' AND policyname = 'Anyone can view badges') THEN
    CREATE POLICY "Anyone can view badges" ON badges_powlax FOR SELECT USING (NOT is_hidden OR auth.uid() IS NOT NULL);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'player_ranks_powlax' AND policyname = 'Anyone can view ranks') THEN
    CREATE POLICY "Anyone can view ranks" ON player_ranks_powlax FOR SELECT USING (true);
  END IF;
  
  -- User-specific policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'user_points_balance_powlax' AND policyname = 'Users can view own points') THEN
    CREATE POLICY "Users can view own points" ON user_points_balance_powlax FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'points_transactions_powlax' AND policyname = 'Users can view own transactions') THEN
    CREATE POLICY "Users can view own transactions" ON points_transactions_powlax FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'user_badge_progress_powlax' AND policyname = 'Users can view own badges') THEN
    CREATE POLICY "Users can view own badges" ON user_badge_progress_powlax FOR SELECT USING (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'user_rank_progress_powlax' AND policyname = 'Users can view own rank') THEN
...
BEGIN
  ...
  LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = table_name AND policyname = 'Service role full access') THEN
