...
export function JWTAuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkAuth = useCallback(async (): Promise<boolean> => {
    try {
      // DEV MODE: Return true for development
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('🚧 DEV MODE: Skipping WordPress auth check')
        // Set a fake user to prevent auth redirects in development
        setUser({
          id: 999,
          username: 'dev-user',
          email: 'dev@powlax.com',
