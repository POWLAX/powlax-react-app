...
export default function EditCustomDrillModal({ 
  isOpen, 
  onClose, 
  drill,
  onSuccess,
  onDrillUpdated 
}: EditCustomDrillModalProps) {
  const { updateUserDrill, loading: updating } = useUserDrills()
  const { user } = useAuth()
  
  // Form state - all fields from the drill
  ...
  // Sharing options - checkboxes for UI
  const [isPublic, setIsPublic] = useState(false)
  const [teamShare, setTeamShare] = useState(false)
  const [clubShare, setClubShare] = useState(false)
  
  // CRITICAL: Store the actual arrays separately to preserve IDs (following strategy pattern)
  const [teamShareIds, setTeamShareIds] = useState<number[]>([])
  const [clubShareIds, setClubShareIds] = useState<number[]>([])

  // Populate form when drill changes (following successful strategy pattern)
  useEffect(() => {
    if (drill) {
      setTitle(drill.title || '')
      setContent(drill.content || '')
      setDurationMinutes(drill.duration_minutes || drill.duration || 10)
      setCategory(drill.category || 'Custom')
      setEquipment(drill.equipment || '')
      setTags(drill.tags || '')
      setGamePhase(drill.game_phase || '')
      ...
      // Age appropriateness
      setDoItAges(drill.do_it_ages || '')
      setCoachItAges(drill.coach_it_ages || '')
      setOwnItAges(drill.own_it_ages || '')
      
      // Visibility
      setIsPublic(drill.is_public || false)
      
      // CRITICAL: Handle array fields properly to avoid "expected JSON array" error
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!user || !drill) {
      toast.error('Invalid drill or user')
      return
    }
    ...
    try {
      ...
      // Create update data - send ALL fields including arrays
      const updateData = {
        title: title.trim(),
        ...
        // CRITICAL FIX: Send arrays, not booleans! (following successful strategy pattern)
      ...
      await updateUserDrill(drill.id, updateData)
      ...
      toast.error('Failed to update drill. Please try again.')
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[700px] bg-white max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-[#003366]">Edit Custom Drill</DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={updating}
            >
              {updating ? 'Updating Drill...' : 'Update Drill'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
